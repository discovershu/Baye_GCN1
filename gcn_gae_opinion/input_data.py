...
def load_data_sub():
    adj = np.load("/network/rit/lab/ceashpc/xujiang/project/Dir_synthitic/syn_adj_100.npy")
    features = sp.identity(len(adj))
    # adj_mask = np.reshape(adj_mask, [-1])
    adj = sp.csr_matrix(adj)
    return adj, features
...
def load_data_pa():
    adj = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/data/adjacency_matrix_ph_milcom.npy")
...
def load_data_dc():
    adj = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/data/adjacency_matrix_dc_milcom.npy")
    # adj_mask = np.load("./data/adjacency_matrix_dc_milcom.npy")
    features = sp.identity(len(adj))
    # adj_mask = np.reshape(adj_mask, [-1])
    adj = sp.csr_matrix(adj)
