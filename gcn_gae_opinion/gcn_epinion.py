...
for epoch in range(FLAGS.epochs):
    ...
    if np.mod(epoch + 1, 10) == 0:
        ...
        outs = sess.run([opt.cost, opt.cost_belief_a, opt.cost_uncertain_a], feed_dict=feed_dict)

        # Compute test loss
        test_cost = outs[1]
        print("epoch:", epoch + 1, "Loss:", train_cost, "mae_belief=", "{:.3f}".format(outs[1]), "mae_uncertain==",
              "{:.3f}".format(outs[2]))
...
for i in range(10):
    ...
    feed_dict = construct_feed_dict(adj_norm, adj_label, features, placeholders, y_test_belief, y_test_un,
                                    test_mask, omega_test, alpha_0, beta_0)
    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
    outs = sess.run([model.belief, model.uncertain], feed_dict=feed_dict)
    drop_b.append(outs[0])
    drop_u.append(outs[1])
belief = np.mean(drop_b, axis=0)
uncertain = np.mean(drop_u, axis=0)
variance_b = np.std(drop_b, axis=0)
variance_u = np.std(drop_u, axis=0)
error_b = masked_mae_np(belief, y_test_belief, test_mask)
error_u = masked_mae_np(uncertain, y_test_un, test_mask)
print("Dropout output:", "mae_belief=", "{:.3f}".format(error_b), "mae_uncertain==", "{:.3f}".format(error_u))
