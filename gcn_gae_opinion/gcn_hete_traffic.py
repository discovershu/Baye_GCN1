...
from gcn_gae_opinion.optimizer import OptimizerAE, OptimizerHete
from gcn_gae_opinion.input_data import load_data_epinion, load_data_pa, load_data_dc
from gcn_gae_opinion.model import GCNModelAE, GCNModelHete
from gcn_gae_opinion.preprocessing import preprocess_graph, construct_feed_dict, sparse_to_tuple, mask_test_edge_epinion, mask_test_edge_opinion
...
flags.DEFINE_string('dataset', 'pa', 'Dataset string.')
...
print("Dataset = ", FLAGS.dataset, "Test ratio", FLAGS.test_rat)
seed = 1
np.random.seed(seed)
tf.set_random_seed(seed)
# Load data
# adj, features = load_data(dataset_str)
adj, features = load_data_pa()
...
if model_str == 'gcn_ae':
    ...
    model = GCNModelHete(placeholders, num_features, num_nodes, features_nonzero)
...
for k in range(30):
    ...
    for epoch in range(FLAGS.epochs):
        ...
        # Run training epoch
        outs = sess.run([opt.opt_op, opt.cost, model.belief], feed_dict=feed_dict)
        ...
        if np.mod(epoch + 1, 10) == 0:
            ...
            outs = sess.run([opt.cost, opt.cost_belief_a], feed_dict=feed_dict)

            # Compute test loss
            test_cost = outs[1]
            b_mse_one.append(test_cost)
            # print("epoch:", epoch + 1, "Loss:", train_cost, "mae_belief=", "{:.3f}".format(outs[1]))
    best_epoch = np.argmin(b_mse_one)
    b_mse.append(np.min(b_mse_one))
    print("index:", k, "best epoch:", (best_epoch + 1) * 10, "bset_belief:", (np.min(b_mse_one)), "run time = ", time.time() - t1)

print("Optimization Finished!")
print("Totle belief : ", np.mean(b_mse))
