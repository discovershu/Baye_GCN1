...
from gcn_gae_opinion.optimizer import OptimizerAE, OptimizerHomo
from gcn_gae_opinion.input_data import load_data_epinion
from gcn_gae_opinion.model import GCNModelAE, GCNModelHomo
...
if model_str == 'gcn_ae':
    ...
    model = GCNModelHomo(placeholders, num_features, num_nodes, features_nonzero)
...
for epoch in range(FLAGS.epochs):
    ...
    if np.mod(epoch + 1, 10) == 0:
        ...
        outs = sess.run([opt.cost, opt.cost_belief_a, opt.cost_uncertain_a, model.sigma_b, model.sigma_u], feed_dict=feed_dict)

        # Compute test loss
        test_cost = outs[1]
        print("epoch:", epoch + 1, "Loss:", train_cost, "mae_belief=", "{:.3f}".format(outs[1]), "mae_uncertain=",
              "{:.3f}".format(outs[2]), "sigma_belief=", "{:.3f}".format(outs[3]), "sigma_uncertain=",
              "{:.3f}".format(outs[4]))
