...
from gcn_gae_opinion.metrics import masked_dir_error
...
flags.DEFINE_float('test_rat', 0.1, 'test number of dataset.')
...
label_1, label_2, label_3, train_mask, test_mask = mask_test_syn_sub_dif(FLAGS.test_rat, T=FLAGS.T)
...
if model_str == 'gcn_ae':
    ...
    model = GCNModelDir(placeholders, num_features, num_nodes, features_nonzero)
...
for k in range(10):
    ...
    label_1, label_2, label_3, train_mask, test_mask = mask_test_syn_sub_dif(FLAGS.test_rat, T=k)
    for epoch in range(FLAGS.epochs):
        ...
        if np.mod(epoch + 1, 100) == 0:
            ...
            outs = sess.run([opt.cost, model.alpha1, model.alpha2, model.alpha3], feed_dict=feed_dict)
            error = masked_dir_error(outs[1], outs[2], outs[3], label_1, label_2, label_3, test_mask)
            # Compute test loss
            print("epoch:", epoch + 1, "Loss:", "{:.2f}".format(outs[0]), "error:", "{:.2f}".format(error))
