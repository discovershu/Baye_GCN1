...
from gcn_gae_opinion.optimizer import OptimizerAE, OptimizerBaserate
from gcn_gae_opinion.input_data import load_data_epinion
from gcn_gae_opinion.model import GCNModelAE, GCNModelBasetate
...
flags.DEFINE_float('test_rat', 0.5, 'test number of dataset.')
...
if model_str == 'gcn_ae':
    ...
    model = GCNModelBasetate(placeholders, num_features, num_nodes, features_nonzero)
...
for i in range(50):
    ...
    feed_dict = construct_feed_dict(adj_norm, adj_label, features, placeholders, y_test_belief, y_test_un,
                                    test_mask, omega_test, alpha_0, beta_0)
    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
    outs = sess.run([model.belief], feed_dict=feed_dict)
    drop_b.append(outs[0])
belief = np.mean(drop_b, axis=0)
variance_b = np.std(drop_b, axis=0)
error_b = masked_mae_np(belief, y_test_belief, test_mask)
print("Dropout output:", "mae_belief=", "{:.3f}".format(error_b))
